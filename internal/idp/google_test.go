...
func TestGoogleProvider_UserInfo(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			os.Setenv("GOOGLE_USERINFO_URL", server.URL)
			defer os.Unsetenv("GOOGLE_USERINFO_URL")

			provider := NewGoogleProvider("client-id", "client-secret", "https://example.com/callback")
			token := &oauth2.Token{AccessToken: "test-token"}

			userInfo, err := provider.UserInfo(context.Background(), token, tt.allowedDomains, nil)
...
func TestGoogleProvider_UserInfo_ServerError(t *testing.T) {
	...
	_, err := provider.UserInfo(context.Background(), token, nil, nil)
